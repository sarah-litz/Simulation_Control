...
from .Scripts.VoleTests import RandomVoles, AttemptMoveVole
...
mode1 = ClosedBox( timeout = 20, map = simpleMap ) 
mode2 = OpenBox( timeout = 20, map = map )
mode3 = ClosedBox( timeout = 60, map = map )
...
sim1 = RandomVoles( modes = [mode1] ) 
sim2 = AttemptMoveVole( modes = [mode2] )
# sim3 = ButtonTests( modes = [mode3], map = map )
...
sim2.simulation_func[mode2] = (sim2.run)
